...
class weixin_spider:
    ...
    def __init__(self, keywords):
        ...
        self.sogou_search_url = 'http://weixin.sogou.com/weixin?type=1&query=%s&ie=utf8&s_from=input&_sug_=n&_sug_type_=' % urllib.parse.quote(
            self.keywords)
        ...
        # excel 第一行数据
        self.excel_data = [u'编号', u'时间',  u'文章标题', u'封面图片', u'文章地址', u'文章简介']
        # 定义excel操作句柄
        self.excle_w = xlsxwriter.Workbook(self.keywords + '/' + self.keywords + '.xls')
    ...
    def get_selenium_js_html(self, url):
        browser = webdriver.PhantomJS(executable_path=r'F:\python\phantomjs-2.5.0-beta-windows\bin\phantomjs.exe')
    ...
    def switch_arctiles_to_list(self, articles):
        ...
        excel_content = self.excle_w.add_worksheet(excel_sheet_name)
    ...
    def parse_one_article(self, article, i, excel_content):
        ...
        # 将这些简单的信息保存成excel数据
        cols = 0
        tempContent = [i, date, title, pic, url, summary]
    ...
    def parse_cover_pic(self, article):
        ...
        rs = p.findall(pic)
        self.log(u'封面图片是：%s ' % rs[0] if len(rs) > 0 else '')
...
if __name__ == '__main__':
    print(u'''
            ************************************************** 
            **    Welcome to Pycharm of weixin gongzhonghao  ** 
            **************************************************
    ''')
    gongzhonghao  = u'vitelabs'
